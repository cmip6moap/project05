...
rcp24_data_dir = "/badc/cmip6/data/CMIP6/ScenarioMIP/*/*/ssp245/*/Amon/ua/gn/latest"
data_in_dir_paths = glob(rcp24_data_dir)
rcp24_models = {}
for i in range(0,len(data_in_dir_paths)):
    model_name = data_in_dir_paths[i].split(sep='/')[7]
    if model_name not in rcp24_models:
        rcp24_models[model_name] = [data_in_dir_paths[i].split(sep='/')[9]] # assign member to list
    else:
        rcp24_models[model_name].append(data_in_dir_paths[i].split(sep='/')[9])

model_id = int(os.environ["model_id"])
model = list(rcp24_models.keys())[model_id]

for member in rcp24_models[model]:
    if model == 'MRI-ESM2-0':
        rcp24_model_data_in_dir_paths = '/badc/cmip6/data/CMIP6/ScenarioMIP/MRI/MRI-ESM2-0/ssp245/r1i1p1f1/Amon/ua/gn/latest/ua_Amon_MRI-ESM2-0_ssp245_r1i1p1f1_gn_206501-210012.nc'
    else:
        rcp24_model_data_dir = "/badc/cmip6/data/CMIP6/ScenarioMIP/*/"+ str(model)+'/ssp245/'+str(member)+'/Amon/ua/gn/latest/*'
        rcp24_model_data_in_dir_paths = glob(rcp24_model_data_dir)
    ds_n = xr.open_mfdataset(rcp24_model_data_in_dir_paths)
    try :
    ...
    outdir='/home/users/cjm317/CMIP6_Hackathon/RCP24'
